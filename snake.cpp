...
void Snake::Init() {
	...
	jaw::Point size = window->getProperties().size;
	graphics->FillRect(jaw::Rect(0, 0, size.x, TILE_SIZE), BORDER_COLOR, 0);
	graphics->FillRect(jaw::Rect(0, 0, TILE_SIZE, size.y), BORDER_COLOR, 0);
	graphics->FillRect(jaw::Rect(size.x, size.y, 0, size.y - TILE_SIZE), BORDER_COLOR, 0);
	graphics->FillRect(jaw::Rect(size.x, size.y, size.x - TILE_SIZE, 0), BORDER_COLOR, 0);
	...
	input->BindKeyDown(jaw::W, up);
	input->BindKeyDown(jaw::UP, up);
	input->BindKeyDown(jaw::S, down);
	input->BindKeyDown(jaw::DOWN, down);
	input->BindKeyDown(jaw::A, left);
	input->BindKeyDown(jaw::LEFT, left);
	input->BindKeyDown(jaw::D, right);
	input->BindKeyDown(jaw::RIGHT, right);
...
void Snake::Loop() {
	...
	graphics->DrawString(L"Score: " + std::to_wstring(snakeParts.size()), jaw::Rect(GAP_SIZE, GAP_SIZE, INT16_MAX, INT16_MAX), 1);
...
void Snake::DrawTile(uint32_t color, jaw::Point tile) const {
	...
	graphics->FillRect(rect, color, 1);
}